...
const char* test_xml01 = R"(<?xml version="1.0" encoding="utf-8"?>
<RestraintLayout
...
const char* test_xml02 = R"(<?xml version="1.0" encoding="utf-8" standalone="yes"?>
<abc:RestraintLayout
    id="@+id/rl_gallery_window_root"
    width="fill"
    height="fill">
...
TEST_CASE(XMLUnitTest) {
    ...
    TEST_DEF("XML parser test 1.") {
        ...
        TEST_FALSE(parser.parse(iss, &root));

        return true;
    };

    TEST_DEF("XML parser test 2.") {
    ...
    TEST_DEF("XML parser test 3.") {
        const char* test_xml = R"(<a></a>)";
        std::istringstream iss(test_xml, std::ios::binary);
    ...
    TEST_DEF("XML parser test 5.") {
        std::istringstream iss(test_xml01, std::ios::binary);
    ...
    TEST_DEF("XML parser test 6.") {
        std::istringstream iss(test_xml02, std::ios::binary);
